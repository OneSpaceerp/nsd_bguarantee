...
def validate_bank_guarantee_type(doc):
    # Condition 1: Bank Guarantee Type "Receiving"
    if doc.bg_type == "Receiving":
        if doc.reference_doctype != "Purchase Order":
            frappe.throw(_("When Bank Guarantee Type is 'Receiving', Reference Document Type must be 'Purchase Order'."))
        if doc.party_type != "Supplier":
            frappe.throw(_("When Bank Guarantee Type is 'Receiving', Party Type must be 'Supplier'."))

    # Condition 2: Bank Guarantee Type "Providing"
    elif doc.bg_type == "Providing":
        if doc.reference_doctype != "Sales Order":
            frappe.throw(_("When Bank Guarantee Type is 'Providing', Reference Document Type must be 'Sales Order'."))
        if doc.party_type != "Customer":
            frappe.throw(_("When Bank Guarantee Type is 'Providing', Party Type must be 'Customer'."))

def on_submit_1(doc, method=None):
    # Call the validation function
    validate_bank_guarantee_type(doc)
